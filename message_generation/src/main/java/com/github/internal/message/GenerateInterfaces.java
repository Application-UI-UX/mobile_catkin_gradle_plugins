...
import com.github.internal.message.definition.MessageDefinitionTupleParser;
import com.github.internal.message.action.ActionDefinitionFileProvider;
import com.github.internal.message.action.ActionGenerationTemplateActionGoal;
import com.github.internal.message.action.ActionGenerationTemplateActionResult;
import com.github.internal.message.action.ActionGenerationTemplateActionFeedback;
import com.github.internal.message.action.ActionGenerationTemplateGoal;
import com.github.internal.message.action.ActionGenerationTemplateResult;
import com.github.internal.message.action.ActionGenerationTemplateFeedback;
import com.github.internal.message.service.ServiceDefinitionFileProvider;
import com.github.internal.message.topic.TopicDefinitionFileProvider;
...
public class GenerateInterfaces {
  ...
  /**
   * @param packages
   *                        a list of packages containing the topic types to
   *                        generate
   *                        interfaces for
  ...
  private void writeTopicInterfaces(File outputDirectory, Collection<String> packages)
    ...
    for (String pkg : packages) {
      Collection<MessageIdentifier> messageIdentifiers = topicDefinitionFileProvider
          .getMessageIdentifiersByPackage(pkg);
  ...
  private void writeServiceInterfaces(File outputDirectory, Collection<String> packages)
    ...
    for (String pkg : packages) {
      Collection<MessageIdentifier> messageIdentifiers = serviceDefinitionFileProvider
          .getMessageIdentifiersByPackage(pkg);
      if (messageIdentifiers != null) {
    ...
    for (MessageIdentifier serviceType : serviceTypes) {
      ...
      MessageDeclaration requestDeclaration = MessageDeclaration.of(serviceType.getType() + "Request",
          requestAndResponse.get(0));
      MessageDeclaration responseDeclaration = MessageDeclaration.of(serviceType.getType() + "Response",
          requestAndResponse.get(1));
  ...
  private void writeActionInterfaces(File outputDirectory, Collection<String> packages)
    ...
    for (String pkg : packages) {
      Collection<MessageIdentifier> messageIdentifiers = actionDefinitionFileProvider
          .getMessageIdentifiersByPackage(pkg);
      if (messageIdentifiers != null) {
    ...
    for (MessageIdentifier actionType : actionTypes) {
      ...
      MessageDeclaration actionDeclaration = MessageDeclaration.of(actionType.getType(), definition);
